...
class RealMachine:
    ...
    def exec_interrupt(self):
        if self.test():  # Tikriname pertraukimus naudodami test()
            if self.cpu.ti == 0:
                print("Timer interrupt triggered! Resetting timer.")
                self.cpu.ti = 10  # Reset taimerio reikšmę
            if self.cpu.pi > 0:
                print(f"Program interrupt triggered: PI = {self.cpu.pi}")
                self.cpu.pi = 0  # Išvalykite PI po apdorojimo
            if self.cpu.si > 0:
                print(f"Supervisor interrupt triggered: SI = {self.cpu.si}")
                self.cpu.si = 0  # Išvalykite SI po apdorojimo

    def test(self):
        if self.cpu.pi > 0 or self.cpu.si > 0 or self.cpu.ti == 0:
            return True  # Pertraukimas aptiktas
        return False  # Jokių pertraukimų
